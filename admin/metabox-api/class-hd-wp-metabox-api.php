...
class HD_WP_Metabox_API {
	...
	function __construct( $options = array(), $fields = array() ) {
		...
		$this->supported_fields = (array) apply_filters( 'hd_metabox_api_supported_fields', array( 'text', 'textarea', 'radio', 'checkbox', 'select', 'multicheck', 'upload', 'color', 'editor', 'section' ) );
	...
	function display_metabox( $post ) {
		...
		echo '<div class="hd-metabox-inner ' . sanitize_html_class( $this->options['metabox_id'] ) . sanitize_html_class( $this->options['metabox_classes'] ) . '">';
	...
	function sanitize_value( $new_value, $field ) {
		...
		switch ( $field['sanit'] ) {
			...
			default :
				...
				return $new_value;
				//return apply_filters( 'hd_metabox_api_sanitize_option', $new_value, $field );
				break;
